...
export default function BlogForm(props: Readonly<BlogFormProps>) {
  ...
  const blogFormSchema = z.object({
    ...
    tags: z
      ...
      .nonempty({ message: "Must have at least one tag per blog" }),
  });
  ...
  async function onSubmit(values: z.infer<typeof blogFormSchema>) {
    if (props.token === undefined) {
      toast({
        ...
        description: toastMessages.tokenUndefined,
      });
    } else {
      ...
      if (res.status !== 200) {
        toast({
          variant: "destructive",
          title: toastMessages.errorMessage,
          description: toastMessages.createBlogFailedDescription,
        });
      } else {
        toast({
          title: toastMessages.createBlogSucceedTitle,
          description: toastMessages.createBlogSucceedDescription,
        });
        reset();
        router.refresh();
      }
  ...
  return (
    <Form {...form}>
      ...
      >
        ...
        />
        {/* body */}
        <MyFormTextArea form={form} value="body" placeholder={"Your Story"} />
        {/* tag */}
        <TagFormField form={form} value="tags" placeholder="tags" />
        {/* submit button */}
        <Button
          type="submit"
          className="w-full cursor-pointer text-white text-md rounded-lg bg-gradient-to-r  from-g3 to-g2 "
        >
          Create
        </Button>
      </form>
